...
export async function fetchFiles(repo, path = '') {
  const res = await fetch(`${API_URL}/api/files?repo=${repo}&path=${path}`, {
    credentials: 'include',
  });
...
export async function fetchFileContent(repo, path) {
  const res = await fetch(`${API_URL}/api/file-content?repo=${repo}&path=${path}`, {
    credentials: 'include',
  });
...
export async function updateFileContent(repo, path, content, sha, message) {
  const res = await fetch(`${API_URL}/api/update-file`, {
    method: 'POST',
    headers: {
